...
# MAGIC %pip install --quiet -U databricks-agents mlflow-skinny mlflow mlflow[gateway] langchain langchain_core langchain_community databricks-vectorsearch databricks-sdk ydata-profiling==4.4.0 pydantic==1.10.18
...
rag_chain_config = {
    ...
    "input_example": {
        "messages": [
            {"role": "user", "content": "query s loy member table. blood pressure column is bp. tell top 2 records from table."}
        ]
    },
    "llm_config": {
...
# result = agent.invoke("get rows from s_loy_member to show everything limited to 3 records")
result = agent.invoke("Was there a stock split for AAPL in the 2014 2015 timeframe?")
print(result['output'])
